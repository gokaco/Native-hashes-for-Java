...
final class Ripemd320Java extends Ripemd320 {
	...
	protected void compress(@Unsigned byte[] msg, int off, int len) {
		for (int i = off, end = off + len; i < end; i += 64) {
			...
			@Unsigned int al = state[0], ar = state[5];
			@Unsigned int bl = state[1], br = state[6];
			@Unsigned int cl = state[2], cr = state[7];
			@Unsigned int dl = state[3], dr = state[8];
			@Unsigned int el = state[4], er = state[9];
